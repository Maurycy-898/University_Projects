...
public final class Compressor3 {
     ...
     */
    public Compressor3(int numberOfBits, BitOutputStream out) {
    ...
    public void write(FrequencyTable frequencyTable, int symbol) throws IOException {
        ...
        while (low > halfRange || high < halfRange) {
            shiftAndEmit();
        }
        // Now low's top bit must be 0 and high's top bit must be 1

        // While low's top two bits are 01 and highs are 10, delete the second-highest bit of both
        while (low > quarterRange && high < 3 * quarterRange) {
            underflow();
            low = 2 * (low - quarterRange);
            high = 2 * (high - quarterRange) + 1;
        }
    }
