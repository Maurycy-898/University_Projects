...
int main(void){
    printf("PROGRAM GRAJĄCY W MASTERMIND\n");   //inicjowanie zmiennych
    const int wszystkie_elementy = 6*6*6*6;
    int czerwone,biale;
    ...
    while(true){                       //głowna pętla,działą dopóki nie zgadnie/wykryje oszustwa
        pozycja = 0;
        while(glowna_baza[pozycja].status!=true)
            pozycja++;
            
        pytanie(&czerwone,&biale,glowna_baza[pozycja]);
        assert((czerwone+biale)<5);                           //zbieranie danych od użytkownika
        if(czerwone == 4){printf("\nmastermind.exe: WYGRAŁEM !!!\n");break;}
