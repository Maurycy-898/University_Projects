...
from utils import QuantErrors, COLOURS
...
def quantize(input_image: str, output_image: str, r_bits: int, g_bits: int, b_bits: int):
    ...
    with open(input_image, 'rb+') as fi:
        ...
        with open(output_image, 'wb') as fo:
            ...
            # wrap the bits counts inside a convenient list
...
def generate_rgb_combinations(pixel_bits_count: int):
...
def main():
    ...
    rgb_combinations = generate_rgb_combinations(pixel_depth)
    ...
    for bit_spread in rgb_combinations:
        print("bit spread ", str(bit_spread))
        ...
        if bit_spread_measure == 'MSE':
            ...
            if max(results.calc_mse(c) for c in COLOURS) < max(best_results.calc_mse(c) for c in COLOURS):
                # if results.calc_mse("") < best_results.calc_mse(""):
                better = True
        else:  # bit_spread_measure == SNR
            # compare the lowest SNR
            # if max(results.calc_snr(c) for c in COLOURS) < max(best_results.calc_snr(c) for c in COLOURS):
            if results.calc_snr("") > best_results.calc_snr(""):
                better = True
        ...
        if better:
            ...
            # report
            print('found better bit spread', str(best_bit_spread))
