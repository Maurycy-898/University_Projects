...
import code.compression_algorithms.ArithmeticCompressor;
import code.compression_algorithms.ArithmeticDecompressor;
import code.tools.FrequencyTable;
import code.statistics.CompressionRatio;
import code.statistics.EntropyCalculator;
import java.io.*;

public class ArithmeticCD_App {
    final static int NUMBER_OF_SYMBOLS = 257;
    final static int NUMBER_OF_BITS = 32;
    ...
    static void compress(InputStream in, BitOutputStream out) throws Exception {
        ...
        ArithmeticCompressor compressor = new ArithmeticCompressor(NUMBER_OF_BITS, out);
        while (true) {
            int symbol = in.read();
            // EOF symbol
            if (symbol == -1) {
    ...
    static void decompress(BitInputStream in, OutputStream out) throws Exception{
        FrequencyTable frequencies = new FrequencyTable(NUMBER_OF_SYMBOLS);
        ArithmeticDecompressor decompressor = new ArithmeticDecompressor(NUMBER_OF_BITS, in);
        while (true) {
